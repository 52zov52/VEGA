...
def main() -> None:
    ...
    try:
        ...
        raise SystemExit(2)
    errors: list[str] = []
    if list(df.columns) != ["anon_polygon_id", "date", "primary_ndvi_pred"]:
        errors.append(f"колонки должны быть anon_polygon_id,date,primary_ndvi_pred, найдено: {list(df.columns)}")
    if df[["anon_polygon_id", "date"]].isna().any().any():
        errors.append("есть пустые anon_polygon_id/date")
    if df["primary_ndvi_pred"].isna().any():
        errors.append("есть пропуски primary_ndvi_pred")
    try:
    ...
    vals = pd.to_numeric(df["primary_ndvi_pred"], errors="coerce")
